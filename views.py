...
class CreateListingForm(forms.ModelForm):
    ...
    price = forms.DecimalField(label="Price", max_digits=10, decimal_places=2, required=True, widget=forms.NumberInput(attrs={
        "class": "form-control",
        "min": 0.01,
        "step": "0.01"
    }))
    ...
    class Meta:
        model = Auction
        fields = ["title", "description", "category", "image_url", "price"]  # Füge 'price' zu den Feldern hinzu
...
def create_listing(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            # Den Preis explizit setzen
            listing = form.save(commit=False)
            listing.seller = request.user
            listing.price = form.cleaned_data["price"]  # Sicherstellen, dass der Preis übernommen wird
            listing.current_price = form.cleaned_data["price"]  # Startpreis und aktueller Preis gleich setzen
            listing.save()
            return redirect('auctions:index')  # Weiterleitung nach erfolgreichem Speichern
    else:
